...
import me.ravriv.lite.mixins.accessors.FontRendererAccessor;
import me.ravriv.lite.mixins.accessors.GlStateManagerAccessor;
...
public final class FontRendererHandler {
    ...
    public FontRendererHandler(FontRenderer fontRenderer) {
    ...
    public void create() {
        ...
        for (int i = 0; i < 256; i++) {
            try (final InputStream stream = mc.getResourceManager().getResource(new ResourceLocation(String.format("textures/font/unicode_page_%02x.png", i))).getInputStream()) {
                ...
                if (i == 0x0d) {
                    try (final InputStream sustream = FontRendererHandler.class.getResourceAsStream("/assets/patcher/font_glyph_data.bin")) {
                        Graphics2D graphics = bufferedImage.createGraphics();
                        graphics.setComposite(AlphaComposite.Src);
                        graphics.drawImage(ImageIO.read(Objects.requireNonNull(sustream)), 0x9DE % 16 * 16, (i % 16 * texSheetDim) + (0x9DE / 16 - 16), null);
    ...
    private Pair<Float, Float> getUV(char characterIndex) {
        ...
        return new Pair<>((row * texSheetDim + charX) / fontTexWidth, (column * texSheetDim + charY) / fontTexHeight);
    }
